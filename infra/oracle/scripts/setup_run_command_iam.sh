...
DG_NAME="${OCI_RUN_COMMAND_DG_NAME:-stock-broker-run-command-dg}"
POLICY_NAME="${OCI_RUN_COMMAND_POLICY_NAME:-stock-broker-run-command-policy}"
...
if [[ -z "${EXISTING_DG_ID}" ]]; then
  ...
  "${OCI_BIN}" iam dynamic-group create \
    ...
    --name "${DG_NAME}" \
    --description "Allows the stock broker instance to use OCI Run Command" \
    --matching-rule "${MATCHING_RULE}" \
    --wait-for-state ACTIVE >/dev/null
else
  echo "Updating dynamic group ${DG_NAME}"
...
if [[ -z "${EXISTING_POLICY_ID}" ]]; then
  ...
  "${OCI_BIN}" iam policy create \
    --compartment-id "${TENANCY_ID}" \
    --name "${POLICY_NAME}" \
    --description "Policies for OCI Run Command on the stock broker instance" \
